# -*- coding: utf-8 -*-
"""
    sphinx.ext.sachi_todo
    ~~~~~~~~~~~~~~~

    This is an edit of Sphinx's todo extension suited for osb-learn's needs.
    Variants:
        - Todos have a priority attached to them.
        - Todos are formatted like traditional admonitions.
        - The output todo item does not expose the directory tree of the file.
...
# There's probably a naming conflict, so the logger doesn't work.
...
class Todo(BaseAdmonition):
    ...
    option_spec = {
        ...
        'priority': todo_node.priority,
    }

    def run(self):
        # type: () -> List[nodes.Node]
        if not self.options.get('class'):
            self.options['class'] = ['todo']
